...
public class CloseBracketFunctionState extends State<FunctionScope> {
    private final Log4jLoggerAdapter logger = (Log4jLoggerAdapter) LoggerFactory.getLogger(CloseBracketFunctionState.class);
    @Override
    public boolean tryTransition(InputCharacterStream characterStream, FunctionScope builder) {
        logger.info("Start transition for Close bracket in function");
