...
export default () => module('renameFile', () => {
  test('Should handle a response with code 200', async (assert) => {
    ...
    const result = await apiService.renameFile(item, newName);

    assert.ok(fetch.called(), 'Should send a request');
    assert.equal(result.name, newName, 'Should get new name of renamed file');
  });
  ...
  test('Should handle a response with code 4**', async (assert) => {
    ...
    try {
      await apiService.renameFile(item, 'name');
    } catch (error) {
